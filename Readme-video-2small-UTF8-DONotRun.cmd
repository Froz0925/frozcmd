@echo off
set "VERS=Froz video recode script v12.08.2025"
...
if not exist "%FFM%" (
    echo(%FFM% не найден, выходим.
    pause
    exit /b
)
if not exist "%FFP%" (
    echo(%FFP% не найден, выходим.
    pause
    exit /b
)
if not exist "%MI%" (
    echo(%MI% не найден, выходим.
    pause
    exit /b
)
if not exist "%MKVP%" (
    echo(%MKVP% не найден, выходим.
    pause
    exit /b
)
...
:: MP4 нужен для записи тега Rotate, MKV нужен для записи metadata color range.
:: Поэтому пишем ошибку и выходим.
