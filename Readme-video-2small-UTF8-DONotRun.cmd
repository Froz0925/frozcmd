...
if not defined LENGTH_SECONDS (
    >>"%LOG%" echo([ERROR] Не удалось извлечь длительность видео
    goto DONE_LENGTH
)
...
if not defined SPEED_CENTI (
    >>"%LOG%" echo([WARNING] Неизвестный кодек: %CODEC%.
    >>"%LOG%" echo(Для расчёта времени кодирования берем скорость 50
    set "SPEED_CENTI=50"
)
...
if defined FORCE_FULL_RANGE (
    set "COLOR_RANGE=1"
    >>"%LOG%" echo([INFO] Задан Force color range
...
if not defined PIX_FMT (
    >>"%LOG%" echo([ERROR] FFProbe не смог определить формат пикселей
    goto COLOR_RANGE_DONE
)
>>"%LOG%" echo([INFO] Формат пикселей исходника: %PIX_FMT%
...
>>"%LOG%" echo([INFO] Для записи metadata full color с помощью mkvpropedit - меняем расширение на mkv
set "OUTPUT_EXT=mkv"
set "OUTPUT=%OUTPUT_DIR%%OUTPUT_NAME%.%OUTPUT_EXT%"
:COLOR_RANGE_DONE
...
if not defined CURRENT_W (
    >>"%LOG%" echo([ERROR] FFProbe не смог определить ширину/высоту
    goto SKIP_CURRENT_DIM
)
>>"%LOG%" echo([INFO] Разрешение исходника: %CURRENT_W%x%CURRENT_H%
:SKIP_CURRENT_DIM
...
>>"%LOG%" echo([INFO] Задан USER ROTATION=%ROTATION%
goto APPLY_ROTATION
...
>>"%LOG%" echo([INFO] В metadata найден тег Rotate: "%ROTATION_TAG%"
...
if "%ROTATION%" == "90" (
    ...
    >>"%LOG%" echo([INFO] Применён поворот на 90 градусов по часовой стрелке
    goto ROTATION_DONE
)
...
if "%ROTATION%" == "180" (
    ...
    >>"%LOG%" echo([INFO] Применён поворот на 180 градусов
    goto ROTATION_DONE
)
if "%ROTATION%" == "270" (
    set "ROTATION_FILTER=transpose=2"
    >>"%LOG%" echo([INFO] Применён поворот на 90 градусов против часовой стрелки
    goto ROTATION_DONE
)
...
if defined COLOR_RANGE (
    ...
    echo(
    >>"%LOG%" echo([ERROR] Конфликт: colour-range и rotate. Смените кодек.
    goto NEXT
)

:: Если OUTPUT_EXT = mkv - меняем на mp4
if not "%OUTPUT_EXT%" == "mkv" goto ROT_EXT_OK
>>"%LOG%" echo([INFO] Меняем расширение на mp4 для записи тега Rotate
set "OUTPUT_EXT=mp4"
set "OUTPUT=%OUTPUT_DIR%%OUTPUT_NAME%.%OUTPUT_EXT%"
:ROT_EXT_OK
...
>>"%LOG%" echo([INFO] Так как поворот кодеком невозможен - тег поворота будет сохранён в файле MP4
goto ROTATION_DONE
...
if not defined SCALE (
    >>"%LOG%" echo([INFO] Высота не задана, масштабирование отключено
    goto SKIP_SCALE
)
set "TARGET_H=%SCALE%"
...
if "%ROTATION%"=="90" (
    ...
    >>"%LOG%" echo([INFO] Задан поворот на 90 градусов по часовой стрелке - масштабируем по ширине
    goto SKIP_SCALE
)
if "%ROTATION%"=="270" (
    set "SCALE_EXPR=scale=%SCALE%:-2"
    >>"%LOG%" echo([INFO] Задан поворот на 90 градусов против часовой стрелки - масштабируем по ширине
    goto SKIP_SCALE
)
...
>>"%LOG%" echo([INFO] Видео будет масштабировано. Исходная высота: %SRC_H%, целевая: %SCALE%
goto SKIP_HEIGHT_CHECK
...
>>"%LOG%" echo([INFO] Высота %SCALE% уже соответствует файлу. Масштабирование пропущено
...
if defined FPS (
    >>"%LOG%" echo([INFO] FPS задан принудительно: %FPS%, пропускаем его извлечение
    goto FPS_DONE
)
...
if not defined R_FPS (
    >>"%LOG%" echo([WARNING] Не удалось найти r_frame_rate
    goto FPS_DONE
)
if not defined A_FPS (
    >>"%LOG%" echo([WARNING] Не удалось найти avg_frame_rate
    goto FPS_DONE
)

:: Сравниваем значения как строки
if "%R_FPS%" == "%A_FPS%" goto FPS_DONE
>>"%LOG%" echo([INFO] Обнаружен FPS VFR. Извлекаем max frame rate из mediainfo
...
if not defined MAX_FPS (
    >>"%LOG%" echo([WARNING] Не удалось извлечь max frame rate из mediainfo
    goto FPS_DONE
)

>>"%LOG%" echo([INFO] Извлечен max frame rate: %MAX_FPS%
...
>>"%LOG%" echo([INFO] По диапазонам установлен FPS CFR: %FPS%
:FPS_DONE
...
:PROFILE_DONE
>>"%LOG%" echo([INFO] Установлен профиль кодирования: %USE_PROFILE%
...
:NO_MAIN10
>>"%LOG%" echo([WARNING] Кодек %CODEC% не поддерживает профиль main10. Параметр проигнорирован

:SET_PIXFMT8
set "PIX_FMT_ARGS=-pix_fmt yuv420p"

:DONE_PIXFMT
>>"%LOG%" echo([INFO] Для кодека %CODEC% с профилем %USE_PROFILE% добавляем флаг: %PIX_FMT_ARGS%
...
>>"%LOG%" echo([INFO] CRF установлен: %CRF%
:SKIP_CRF
...
if defined VF >>"%LOG%" echo([INFO] Видеофильтр: %VF%
...
>>"%LOG%" echo([INFO] Установлен preset %PRESET%
:SKIP_PRESET
...
:: Меняем язык дорожек на русский кроме видео. 
:: Для MKV - добавит позже mkvpropedit. Он же добавит метадату для full-range MKV - ffmpeg не умеет.
:: Язык видео в ffmpeg не трогаем - он это делает криво. В MP4 останется und/eng - пусть.
...
>>"%LOG%" echo([INFO] Full color range: добавляем в MKV дополнительные теги c помощью mkvpropedit
>>"%LOG%" echo([INFO] В MKV меняем язык видеодорожки на русский
...
if %ERRORLEVEL% EQU 0 (
    ...
    echo(
    >>"%LOG%" echo([ERROR] FFmpeg завершился с ошибкой - см. "%FFMPEG_LOG_NAME%"
)
del "%TMPFLOEM%"
popd
del "%VT%"
